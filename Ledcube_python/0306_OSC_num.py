...
class OSCSenderApp:
    def __init__(self, master, ip1, ip2, ip3, port1, port2, port3):  # 서버를 지정.
        ...
        self.label = tk.Label(master, text=" [ LED CUBE Control , Set start time]", font=("Arial", 30, "bold"),
                              bg="white")
        self.label.pack()
        ...
        # '10초뒤' 버튼 추가
        self.ten_seconds_button = tk.Button(master, text="5s later", command=self.set_time_ten_seconds_later, width=10,
                                            height=1, font=("Helvetica", 15, "bold"))
        self.ten_seconds_button.pack()
        self.ten_seconds_button.place(x=700, y=70)  # 원하는 위치로 조정

        self.led_off_button = tk.Button(master, text="led off", command=self.led_off, width=10, height=1,
        ...
        # 숫자 입력 칸 추가
        self.number_entry = tk.Entry(master, font=("Helvetica", 20, "bold"), bd=3, relief="solid")
        self.number_entry.insert(0, "Enter number")
        self.number_entry.config(fg="gray")
        self.number_entry.bind("<FocusIn>", self.on_entry_click_number)
        self.number_entry.bind("<FocusOut>", self.on_focus_out_number)
    ...
    def on_entry_click_number(self, event):
        if self.number_entry.get() == "Enter number":
            self.number_entry.delete(0, tk.END)
            self.number_entry.config(fg="black")

    def on_focus_out_number(self, event):
    ...
    def confirm_time(self):
        ...
        try:
            ...
            self.status_label.config(text="Invalid time format. Please enter again", bg="white")
    ...
    def recheck_time(self):
        ...
        self.status_label.config(
            text="Entered time is earlier than current time. Please enter again", fg="red", bg="white")
        self.blink_entry_background(self.time_entry, times=1)
        # 확인 버튼을 다시 처음 상태로 설정
    ...
    def send_number_as_osc_signal(self):
        number = self.number_entry.get()
        try:
            number = int(number)
            self.send_led_signal(number)
        except ValueError:
            self.status_label.config(text="Invalid number. Please enter an integer.", bg="white")
            self.status_label.after(1000, lambda: self.status_label.config(text="", bg="white"))
